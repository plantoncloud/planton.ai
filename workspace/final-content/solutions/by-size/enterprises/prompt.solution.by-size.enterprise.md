I've been working on creating content for our new sales website, and our workflow is to for me as a product owner, to
...
## FOCUS OF THIS REQUEST - Solutions - By Size - Enterprises

Most importantly put yourself in the role of a highly skilled developer tooling sales messaging creation expert for this
task.

## transcription for enterprises

All right, so we move on to talking about how planned on Cloud can actually be suitable for enterprises. So what are
enterprises? According to me, I think enterprise are the companies who have established products with with a large
customer base. So for them, requirements change beyond innovating faster and finding product market fit or finding their
first large customers, it's reliability becomes more important. Security becomes far more important. Compliance becomes
far more important. Availability becomes far more important, and probably dedicated account management and dedicated
support and any service level agreement options for higher availability, data encryption, etc, all of these become far
more important than getting started quickly, or managing teams, etc. So yeah, there is, there is a lot that we can offer
for enterprises. And in fact, right now, plankton cloud is aiming at becoming the default solution for enterprises, more
than startups and growing teams, while keeping the simplicity still as as the way to do things we want to cater to
enterprises, but not at the cost of sacrificing, sacrificing the simple simplicity. So what do we do is, yeah, we do
encrypt all of the data in transit and at storage level, although we are not actually doing it right now, but there is
what that is already at the high priority which we are going to do, but we don't have to. We can claim that we encrypt
everything, because that is what we actually are going to do anyway. And as for Compliance Certification, we take
getting those certifications very seriously, and we are going to definitely work towards getting GDPR, HIPAA and stock
compliance very soon. So if there are any customers who can, who like the offering and are only blocked by a particular
certification, we can definitely accelerate our accelerate the work that is required to get those certificate, but we
just have to express that intention on our website, that we are committed to getting as many compliance certifications
as possible as we are committed to making this an enterprise grade application. So to that, to that end, we are always
working on building a secured solution so that getting these certifications will eventually be a possible option. Now,
advanced automation and integration, this is where platinum cloud can actually shine further, which is the service hub
within plant on Cloud, not just provide out of the box templates like cookie cutter templates or GitHub actions,
workflows, GitLab templates. We are keeping all of these options open for extension so that these enterprises find the
solution applicable for their requirements. So essentially, platform supports bringing or resisting the organization
maintained templates and resisting them so that their developers, when they are bootstrapping services, they can use the
templates created by their organization, with all the best practices built in, more suitable and more compliant for
their own requirements, not just the cookie cutter templates that bootstrap projects, GitHub actions, get workflows,
GitLab templates and more coming. So the organizations can register all of these, and those will surface in the
templates library. So developers can either choose to use plant on clouds provided templates, or they can, or they will
be for like the organizations can make the default choices to be the ones that they create in the form of template
libraries, and now coming to the deployments itself. This is another area where plankton cloud really change, which is
the multi cloud deployment framework on plankton. Cloud is backed by an open source project called Project Plan Tom,
which offers the ability and on top of this, Project Plan Tom, we built what is called a pulumi module registry where
the organization can allow their developers to use the plant or cloud maintained and provided Columbia modules. However,
we imagine that enterprise, the environments in enterprises, are going to be far more complex than when compared to like
startups and growing companies. So we recognize this requirement, and we allow these enterprises to customize or even
build those pulling modules from ground up and register them in the pulley module, register on plant on cloud and make
them the default pull you modules for deploying those components. So while planting cloud offers all the self service
options for developers, but organization can still control how the what would be, the structure, form and shape of those
deployments on their environments. They can full take full control, because they they'll be able to fully own those
pulling modules just by registering and configuring the stack job settings at organization level, organizations get to
take advantage of the self service capabilities combined by the custom pulumi modules that are tailored for their own
organizations and for as for complaints, plant abroad makes it Easy to bring their own pulumi backend state managers, or
even they can run the stack jobs in their own infrastructure to enable complaints. And as for high availability and
reliability, one of the things that enterprises can enjoy is plant on Cloud can be deployed in their own infrastructure,
so that they get to plant on Cloud meets their security and compliance requirements, so that their data never leads the
network boundary. But this is going to be an engagement between plant and Cloud team and the organization, and this
definitely guarantees more uptime and higher performance and also more security because it's not a shared the tenant
anymore. So all of these definitely make plankton cloud accompanying choice for the enterprises.
