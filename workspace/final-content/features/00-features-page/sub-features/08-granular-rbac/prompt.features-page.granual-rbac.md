...
Here is a transcription from one of the videos I recorded explaining a sub-feature of our platform: "Granular RBAC"

```
Okay, so in this discussion, we are going to focus on the governance and administration aspect of an internal developer
platform. So typical organizations would have individuals performing various roles within the organization. Examples of
those individuals with roles would be developers as individual contributors or managers or senior engineers or
architects or leadership team support staff, so that there can be any number of individuals who are part of an
organization, and in many cases, each each of those individuals will be given access to the internal developer platform.
So for the purposes of compliance, governance and administration, it is important to have robust access control
mechanism, usually powered by identity and access management systems. While it is possible to implement a strong, robust
role based access control, the traditional methods are very complicated to implement, and start to hit scaling
limitations with as the number of users grow and as the number of resources that need to be protected or access should
be managed to grow so Google has solved this problem effectively, which we can find on Google Cloud or Google Drive or
any other Google service. So the identities on Google are all Google workspace accounts, or Google gene account,
individual accounts created via Gmail, or even the service accounts created on Google Cloud. So these are three
categories of identities, but they're all in the same identity space. Permissions can be granted to any of these
identities, to any of the resource. When I say resource, we are talking about one of the several one document in
reliance and reliance of Google documents stored on the internet. As soon as you add any of the identity from any of
these categories on Google, instantly, the identity gets access, and also he gets to only perform operations that are
authorized for the assigned role. Google's identity management system is very robust and works really well. At the same
time, it is very intuitive to use. Google released a white paper called Zen zebra, explaining the architecture behind
this sophisticated yet simply identity system that they have built that powers all of these operations on internet. So
when quantum flow was being built for as an internal developer platform, we wanted to design and implement the identity
and access management system to support the governance and administration requirements as Google and we were fortunate
to come across open FGA, which is a technology or an implementation of the Google white Paper. At the end of the
implementation the problems that the IAM implementation on this internal developer platform currently solves are every
new user created on the platform is treated as an identity, every API client created from the console app is also
created as an identity, and every action performed by these identities are authorized by the back end system, and the
authorization works similar to how Google authorization works any identity, unlike Google workspace or Google Drive or
documents, in the context of Google Cloud, There is this notion of organization and product project and similarly,
quantum Cloud has organization and environment context. So when an individual identity account on Google Cloud is
authorized to perform certain operations on a particular resource within a Google Cloud project, then the permissions.
Permission boundary is limited to the context the context being the project. The same rules apply even on planton
Cloud, identities are authorized to perform operations, and those operations are bounded within the context. So to put
in put it into perspective. Movie, Cloud has organizations. Plantain, Cloud has organizations. Google Cloud has projects
under an organization. Planton Cloud has environments under organization and deployment components get deployed to
environments. So for example, if a Redis Database is deployed on Kubernetes on planton cloud, that Redis deployment
belongs to an environment, so only those individuals who have create or deploy permissions for Redis on Kubernetes API
resource are authorized to deploy Redis to Kubernetes. In order for this work, every deployment component has three
roles associated with it. So the deployment components can be examples can be route 53 DNS zone s3 bucket, Google Cloud.
DNS zone, micro service on Kubernetes, solar on Kubernetes, GCP secrets manager. So all of these have been modeled as
ATA resources, and there is three roles, Iain roles defined for each of these API resources. And all future API
resources that are going to be added to the system will also come with three different roles, Owner, viewer and admin
and and when the identities are authorized to perform or authorized with a given role at an organization level, then
that means that identity can perform the operations that are applicable for a particular API resource in the entire
organization. Same goes with environment. So if an identity is granted access to a resource at environment level, then
that identity can perform any resource within that environment, of that resource kind, but the granular RBAC excels when
a particular identity is accessed, is authorized to perform operations on a particular resource. So if an individual is
granted again role for a particular Redis on Kubernetes deployment, then that identity is only going to be allowed to
perform admin operations on that particular radius. So if the environment has other radius on Kubernetes requirements,
the identity will not have permissions to mess with them. So this is what we refer to as granular on back to simplify
the permission management in groups, which is akin to the teams within an organization. Planton Cloud also supports the
concept of teams. Teams represent a group of identities, and permissions can be granted to teams as well. So overall,
this concludes the explanation for the permission management and how well it is. It has been implemented on plant, on
cloud. This particular transcription is aimed at providing chatgpt With all the relevant information that it can pick
when creating sections on the Features page for the granular RBAC feature in particular, it doesn't have to talk about
the open FGR or any other implementation details, but it can certainly draw comparisons between the simplicity of Google
Cloud permission, management, and quantum cloud permission, management, to show the sophisticated yet. This simplicity
of the IAM implementation or granular RBI implementation of plan prompt. 
```

Now you will give me three sections that clearly explain the "Granular RBAC" feature in a way that any
