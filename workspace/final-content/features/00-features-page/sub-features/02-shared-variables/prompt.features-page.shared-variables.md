...
Here is a transcription from one of the videos I recorded explaining a sub-feature of our platform: "ConfigMaps for
Shared Environment Configuration"

```
So internet developer platforms help developers deploy applications that they develop, and these applications can be can
be composed of web applications like next Yes, apps or back end API services developed in Java, Golang or other
programming languages. Or these can be cron jobs for generating things like reporting. Or there can be simply a set of
functions that can be deployed to lambda. So any distributed application today is composed of several of these
components, and each, each of these type of each of these components, when deployed, they also rely on a set of
environment variables or Even environment secrets, right? So? It is a common it is common to it is very common to see
that the configuration that these applications depend on. There is so much duplication around it, like, for example,
let's take two different micro services called payments and transfer, and whenever a payment is successful, an email
gets sent. Whenever a transfer gets successful, an email gets sent. But if there is a survey third party service like
SendGrid that takes care of takes care of sending the emails, then SendGrid account information needs to be configured
in both payment service and transfer service. And whenever the any of these configuration changes, both payment
configuration in payments need to be updated, and configuration and transfer micro service repository should also be
updated so and this starts to get more and more painful with a duplicated configuration across several repositories, as
the size of the team and the number of components keep growing, this configuration space will start to create more of a
problem, maintenance problem, and to to eliminate this Problem, plant on Cloud has provide provides a way for developers
to declare this shared environment variables, uh, via a resource called config map. Akin to this is similar to config
map on Kubernetes, where the data part of the config map is a key value set of key value page, or a map of key value
base. So is the config map API resource created on planton also is a key value map. So developers can create a config
map with all of these shared environment variables and when, when configuring deployments for these applications in
GitHub repositories, with the help of customized pattern, they will be able to set the values for these environment
variables as references for the keys inside this config map resource and during deployment plan down cloud system will
transform these config map references with their values before they get deployed onto the target environments. With this
approach, if the value, let's take, for example, payments and transfer scenario, uh, developers, instead of hard coding
the configuration values associated with SendGrid in each of these repositories, developers will be able to create a
config map, API resource on quantum cloud, configure the SendGrid configuration as key values like semi grid usernames
and grid hosts and grid like what template ID, etc, kind of send. These are not sensitive fields. If this information is
sensitive information. These should be injected via the secrets manager resources, which is a different concept
altogether, which is elaborately explained in the secrets manager media series. But this is always more suitable for non
sensitive configuration that needs to be across to you in several components. So if the sender information is changed
only the values need to be updated in the config map API resource of planton cloud, and as long as the these resources
are redeployed during the redeployment, they get the updated values automatically, thereby eliminating the need for
making code changes within each of these repositories, wherever standard grid configuration is used. This starts to get
more powerful as the team and size of the applications scales, and developers are allowed to create any number of any
number of sets of this configuration config maps by creating new config map resources within the same environment. And
so, for example, developers can create something like send config map with name, send grid config, right? So this only
contains the configuration for send grid, and developers can create config map for another application like Aug zero,
right? So this allows the developers to segregate this reusable configuration into multiple config maps, and when
referencing the configuration for as a value for an environment variable in the customized config interpolated syntax is
used like C map, hyphen the idea of the config map that contains the value For this environment variable and also
specifying the key inside this config map that actually contains the value of for that particular environment variable.
Like, for example, the ENV can have send grid host as the environment variable name, whereas the value is interpolated
dollar curly brace C map followed by the ID dot, sendit, host inside the inside the config map and plan on cloud system
will do the transformation of these interpolated key values during the deployment, thereby ensuring that the values,
when they are applications are deployed onto their environments, like Kubernetes or fargate, they actually have the
values. Those are actual values of the configuration, instead of these interpolated keys. So overall, this reusable or
shared environment variables feature allows the developers to centralize their configuration used in environment and use
that centralized configuration In the when defining their API resource, specific to the deployment on platform. 

```

Now you will give me three sections that clearly explain the "ConfigMaps for Shared Environment Configuration" feature
