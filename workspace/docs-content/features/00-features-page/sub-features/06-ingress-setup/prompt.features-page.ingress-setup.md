...
Here is a transcription from one of the videos I recorded explaining a sub-feature of our platform: "Ingress Setup"

```
All right, one of the complex aspects of deployments that many struggle to understand, including seasoned DevOps
experts, is the setting up of ingress for the applications that are deployed onto environments. First of all, it's
probably just for the sake of understanding the overall topic that we are going to discuss. Let's spend a couple minutes
understanding what is in us. So let's imagine on a remote data center, on a remote server service gets deployed. This
can be a web app like next days, or this can be an API between Java or other languages. Now, what is ingress is any
client that is outside the network boundary of this data center. In order to reach this reach it's the target API, the
server on which the service is running. The first only way to do that is via the IP address. But these IP addresses tend
to change frequently, so instead of using IP it is recommended to most, mostly use DNS names, because these are easy to
remember, and even if the IP address behind the scenes is updated, the DNS name can still work, etc. So the possibility
of connecting to these remote servers from outside the network boundary using DNS names is popularly referred to as
Ingress. Doesn't mean that ingress is not required when both the client and server are within the same network boundary,
even here, if the if a client wants to connect to a server, then the client is still needs to know the DNS name or IP
address for the server and DNS names within the network boundary also provide the same benefits as they provide from the
outside. So the deployment process, the part of the deployment process that ensures that these DNS names are created
which resolve to the IP addresses of the servers on which these services are running. Is referred to as ingress
management. Okay? And Ingress, by its nature, is very complicated because networks in general are hard, and for
developers, it is far more complicated because, again, because networking is a complex subject, burdening developers
with understanding how ingress works and taking care of configuring ingress is not a recommended way to allow developers
to work with higher productivity to solve this problem without paying for developers. The API resources modern for each
of each kind of application is type of workload, like micro services or otherwise, the deployment components which are
for which are modeled based on the workload kind and the target environment. Example is micro service, micro service
Kubernetes right and micro service fargate or Redis micro Redis Kubernetes, notice that we are not just talking about
ingress management for just the services developed in house or applications developed in house. We are also talking
about the ingress management for open source. For example, if you happen to deploy Redis on either easy to instance or
on Kubernetes, in order to reach that, like, for example, we have a Kubernetes cluster, and you have a part that runs a
Redis Database, which is deployed using plantain codes deployment components. In order for like say Suresh, the backend
developer, is working on his laptop to and he is working on a service that requires to connect to Redis, right? And
Suresh wants to know, how can he configure the application running on his laptop to connect to Redis, so which is also
an open source project, but was deployed because his application depends on this open source project. So here the
ingress comes into play. So even though open source components deployed by the developer, DevOps engineers on Kubernetes
using planton cloud, offers a simple mechanism to provide Ingress. So developers simply have to specify the domain name
and the ingress can be and developers can optionally disable or enable Ingress, depending on the whether or not these
particular components require ingress from external systems. So if they would like, they can enable and they can specify
the domain on which these open source or micro in house, developed services require Ingress, and the ingress management
for Kubernetes environments and non Kubernetes environments is also consistent, and the logic for creating the
appropriate resources for these ingress ingress management is is hidden away or is delegated to pulumi modules. So all
the default pulumi modules that are that are available for each deployment component contains the logic for Ingress.
Ingress setup, if the organizations have more special requirements on how the ingress is set up. They can fork these
pulumi modules, customize them, register those pulumi modules, and have the developers always use the customized pulumi
modules. And all of these are capabilities are available for the organizations. So when we're talking about Kubernetes
deployments, if the GK cluster or Eks cluster, AKs cluster are deployed by client on Cloud, then the GK cluster will
have the corresponding components that are needed on Kubernetes, which are Istio for host head, host based routing or IP
based routing, and also search manager, yeah. That brings me to another important topic that is that is far more complex
within the network. Sorry. Index management is provisioning of SSL certs and setting up automatic rotation in Kubernetes
world that has been pretty much standardized with cert manager, yet it is the developers are tasked with, say, setting
up or creation of these certificates. It's a whole it adds a lot more to the complexity. So all these pulumi modules,
say, for example, that DK cluster, when deployed, it creates Istio operator, and it also creates a certain manager and
configures the permissions that are required to update like manage DNS records for provisioning the certificate. All of
that complexity is hidden away from the developers and but the organization still will have more control over the
possibility of customizing pulumi modules when deploying VK cluster, eksc, etc. Now, when applications get deployed, the
pulumi modules corresponding to those deployment components create certificate resources, Certificate Request resources
and configure Istio, virtual service, routes, etc. So the whole solution put together will ultimately simplify the
process of setting up ingress for whether it is in house, built applications or open source deployments. It is pretty
standardized and works very well out of the box. 

```

Now you will give me three sections that clearly explain the "Ingress Setup" feature in a way that any
