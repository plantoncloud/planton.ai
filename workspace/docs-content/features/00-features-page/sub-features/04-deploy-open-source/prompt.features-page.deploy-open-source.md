...
Here is a transcription from one of the videos I recorded explaining a sub-feature of our platform: "Deploying OpenSource Software"

```
So we understand that most of the deployment automation systems today that helps developers or DevOps engineers deploy
software onto onto Main infrastructure. Most of them that you come across are heavily focused on the building and
deploying in house applications or services. These can be web applications or back end API services or cron jobs, so all
of these types of workloads are primarily for created by the developers of the organization that builds these
applications. So these are created by the devs, right? So wherever it is very, very common to find lots and lots of
tools out there that focuses on streamlining the deployment automation for these types of workers. However, very few
tools provide a seamless, streamlined and automated approach for for deploying open source software now, like, Why? Why?
Why is there no solution that kind of provide a seamless and first class support for deploying open source software.
When I say first class support, I mean the deployment automation software that you come across for deploying these kinds
of application do have first class support for these workloads. And when I say first class support the UI is customized
to helping developers deploy these in house built applications and services, etc. So the UI, you will find information
related to the attributes or configuration for the in house built software, like, for example, the in house built
software may request for required build process before deployment happens, etc. And the images for these are usually
published to the private repository, so the image full secrets is already set up with the private Docker registry, etc.
So and now, why is that there is no deployment there is not a whole lot of deployment automation systems that provide
first class support for open source. A couple of examples of the importance of open source software is imagine building
an application without the use of Postgres database. Assume that Postgres and Redis and other open source software like
Kafka, imagine they don't exist, right? It is very hard to build robust applications in modern distributed systems
without these kind of open source without the help of these open source products when it is so common that most of the
applications built in today's time and age, every software or application being built depends on these open source why
is it that the developers are not provided with First Class support for deploying these open source files, the best
thing that can be is available for developers to get. First Class support for deploying these open source softwares is
the cloud provider managed service options so the postgres database can be easily deployed using AWS RDS with Postgres
chosen as the agent. This is by far the simplest and most elegant experience that developers can get to deploy these
open source solutions. However, in most cases, these managed services options are expensive than they should be, meaning
if you run, if you are able to deploy this Postgres, open source database all by yourself, then you're not paying that
additional premium fees for AWS to do. I'm not definitely there are lots of advantages for other advantages, other than
deployment simplification for a managed service, like, for example, you get global replication, real time replication,
Automatch and backups, etc, but in a lot of cases and for a lot of applications, and in fact, for lots of lower
environments, those are not essential. You need that kind of sophisticated and highly reliable systems when you are
running in production and when you're running at scale, but there are today. Kubernetes has become such a good alternate
option for deploying these open source software, and it would be the bin packing nature of Kubernetes makes it possible
to deploy several open source projects on the same on the leftover compute that you can take advantage of, and that
drives cost a lot then. And this is well, very well known at this point, and it has been proven several times. However,
organizations continue to rely on these managed services because, because of the fact that there is no well established
first class support for deploying these open source onto Kubernetes. So that is one of the problems that planton cloud
aimed at solving. And today, planton cloud makes it very, very easy to deploy these open source software onto
Kubernetes. Well, Platinum cloud is set up to help developers deploy most of the open source software, if not all, not
just onto Kubernetes. The pattern can be extended to deploying them onto Kubernetes, like easy to or even sometimes
fargate, etc, etc. But if there is a need, we can extend the pattern. But right now Kubernetes, because it is such a
good platform to deploy open source projects. Plan on Cloud relies on the open source part project plan term to provide
a much seamless experience. So project plan Tom is essentially a unified and consistent API framework for deploying any
type of open source or third party or in house services or multi cloud infrastructure or managed services project plan.
Term is a unified platform to deploy any of these types of workloads, and open source is a core part of this project.
Platform further provides a much more enhanced experience by providing a deployment component store, which allows our
developers to browse what open source projects are available for deployment and what is the input required for from them
in order to deploy, deploy these open source projects. And this is very well integrated into plant on clouds, integrated
IAC workflows, powered by stack jobs. And what is even more awesome is that all of this can also happen where plantora,
the DevOps copilot, via natural language conversational interactions, so developers can simply say something like, Hey,
I need a Redis Database, right? Reduce database, and in less than three minutes, you will get well, weighted out Redis
Database required on your Kubernetes with all the information that you may need to connect to that Redis Database with
ingress also created, etc, so lots of complexity is extracted away from the developers, but also the DevOps engineers
within the organization can further take, take further control in how the Redis is getting deployed by customizing the
existing pulumi Open Source pulumi module for deploying this Redis onto Kubernetes. So although this is create, this is
created via chat interface. The rest of the team and organization will know that you have deployed a Redis Database with
so and so resources on Kubernetes. And you can further tweak this deployment by by continuing your chat in plantora, by
saying, I would like the memory to be increased from two GB to eight GB. Then plan to run, will understand your request
and trigger another pulumi workflow, which shows you the preview and and the overall pulumi up operations right within
the chat, chat terminal. So how would the world look like without plant on Cloud providing a first class support for
these open source project entrepreneurs is that developers have to go and browse heavy repository on artifact, artifact
dot hub, artifact hub.io, or on GitHub. They need to go. Developers need to Google for if there is a very well
maintained Hill chat for that the open source component that they need to and there is usually multiple and so it
should, the onus is on the developer to choose what can work for him, and then he has to go through the documentation.
And the location of the documentation depends on where the maintainer of that project has forced so developers, when
trying to deploy multiple open source projects required in their applications, they come across an inconsistent and
frustrating experience, which is fully addressed with the consistent experience provided the planton cloud, because all
the protocol definitions for each of these open source projects have been like clearly written in protocol language, and
the documentation is published on buff schema industry, making the documentation providing a central place for
developers to browse and explore the like the input specifications that are that plan down cloud allows the developers
to provide, etc, now topping that with the customizability of pulumi modules gives organizations more control over how
the open source software is being being deployed onto Kubernetes, etc. So When? When? When this less focused area of
helping developers deploy open source software with few clicks or even with chat interface definitely is going to
further help developers innovate faster, because when they are provided with all the tools they need, when they need it,
they they tend to be more motivated and work with more enthusiasm and passion. So we think that the open source
deployments, streamlined open source deployments, on plant, on Cloud, will definitely help organizations provide their
developers with much more enhanced experience. for them to succeed. 

```

Now you will give me three sections that clearly explain the "Deploying OpenSource Software" feature
in a way that any developer
