...
## FOCUS OF THIS REQUEST - Solutions - By Role - DevOps or Platform Engineers

Most importantly put yourself in the role of a highly skilled developer tooling sales messaging creation expert for this
task.

## transcription for devops or platform engineers

Let's move on to how platinum cloud offers a good solution for DevOps engineers or platform engineers. From the outset,
it may seem like platinum cloud actually replaces or obviates the need for setting up maintaining DevOps teams or
platform engineers, but that is definitely not the message that we want to convey to the visitors of the website,
because plan, because plan term cloud, is a solution that is eventually going to be evaluated by DevOps or platform
engineers, either at small startups or at growing teams, or even at enterprises, if we build our sales messaging in such
a way that it kind of replaces their jobs, then there is going to be a lot of pushback from these teams, which makes
which is going to make our sales process a lot harder. So I want chatgpt to make sure that it builds the sales messaging
in such a way that plant on cloud actually makes enables DevOps and platform engineers to be more successful in their
job, and helps them provide much more enhanced experience for their developers with the help of plant on cloud. So it
should not, it should not create the messaging that is going we are going to put on the website is not, should not be
like it is going to replace DevOps or platform engineers, right. So now in this discussion, I'm going to talk about all
those things that DevOps engineers and platform engineers can get excited about. So the first thing is, most of the
DevOps and platform engineers love creating automation, but what DevOps or platform engineers are commonly not big fans
of, to create documentation to so that developers can leverage that automation. So basically, creating adoption is a
challenge for DevOps or platform engineers because of the fact that adoption requires extensive documentation and
training for the developers to leverage that automation. So plan term cloud offers much rock solid solution for those,
those developers, and we can include the illustration video that we got created from gaming studios on this page, which
basically talks about how DevOps, platforming needs can create the automation, but not have to deal with spending time
and energy on documenting or training to make sure that there is adoption for the automation that they create, and this
is achieved with the concept of pulumi module registry and All of the open source portion of Project Plan talk. So we
are talking about the deployment portions and also the template library, which enables these platform engineers or
DevOps units to create reusable CICD code and register them on plankton cloud web interface so that it is browsable, and
other team members can explore the modules that are created by platform engineers, so that the developers know where to
look for and also, because of plan Torah, the DevOps co pilot for developers also makes it very easy for developers to
discover the automation created by the platform engineers or DevOps teams, right? So ultimately, the goal of platform
engineers is to provide all the developers self service capabilities by offering them a full fledged application.
Building this from scratch is going to be a very big challenge for startup or growing teams. Maybe the enterprises can
afford to put together a team of platform engineers, but even in those cases, building sophisticated RBAC and security
and like auditability are going to be a huge free battery for those organizations, and are going to be a waste of
energy. So platform engineers can definitely get started with plant on cloud and but customize every single pulumi
modules that are required within their organization and gain gain advantage of the self service capabilities already
offered by plant on code so they don't have to spend any energy in adoption. So all of the automation that they create
the adoption is going to be much higher when they use plant on Cloud, etc. And plant on Cloud also offers the ability to
create third party plugins, eventually, so platform engineers can probably create plugins that are going to be suitable
for offering customized functionality for their own organizations. And platform cloud offers, uh, out of the box, SDKs
that that platform engineers can hook into and they can write custom programs to actually offer build more user
interfaces if they need and we plan on Cloud offers the ability to create client credentials for plant on Cloud APIs.
And because we use protobuf and buff schema industry, there are the language SDKs are also available for all the plant
on Cloud APIs, so it is possible for them to create custom tools to enhance the functionality already often by plant
down flow so.
